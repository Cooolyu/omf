package com.gmsz.om.web.snmp.controller;
...
public class SnmpController {
    ...
    public SnmpController(int version) {
        try {
    ...
    public void sendMessage(Boolean syn, final Boolean bro, PDU pdu, String addr)
        ...
        if (!syn) {
            ...
            Iterator it =  response.getResponse().getVariableBindings().iterator();
            while(it.hasNext()) {
    ...
    public static void main(String[] args) {
        ...
        SnmpController manager = new SnmpController(ver2c);
        // 构造报文
        PDU pdu = new PDU();
         //PDU pdu = new ScopedPDU();
        // 设置要获取的对象ID，这个OID代表远程计算机的名称
        OID oids = new OID(".1.3.6.1.4.1.15227.1.3.1.1.1.0");
        OID oids2 = new OID(".1.3.6.1.2.1.1.5.0");
        ...
        try {
            ...
            manager.sendMessage(false, true, pdu, "udp:192.168.18.70/161");//192.168.1.233 WinServer2008服务器
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
